...
def view_viz(infrastructure, jobs, work=True, stylefn=cmdstyle, current_user=None):
    ...
    if not infrast_down:
        ...
        highlighted_users += pd.DataFrame([(j.user, sum([x.n_gpus for x in j.joblets])) for j in jobs if j.user != current_user and j.state.isin('R', 'S')]).groupby(0).sum()[1].sort_values(ascending=False).iloc[:3].index.to_list()
